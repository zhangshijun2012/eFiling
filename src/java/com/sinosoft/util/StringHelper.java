...
public class StringHelper {
	...
	/**
	 * 字符串的字节长度,与系统编码有关.如果是UTF-8编码则一个中文为3字节,GBK则为2字节
	 * 
	 * @return
	...
	public static String join(Object[] array, String separator) {
		...
		for (int i = 0, l = array.length; i < l; i++) {
			...
			if (o == null) {
				...
				o = StringHelper.join((Object[]) o);
			} else if (Iterable.class.isAssignableFrom(o.getClass())) {
				o = StringHelper.join((Iterable<?>) o);
			} else if (Map.class.isAssignableFrom(o.getClass())) {
				o = StringHelper.join((Map<?, ?>) o);
			}
			v.append(o);
		}
	...
	/**
	 * 截取字符串，注意此处字符串的长度为其字节长度，即一个中文字符占2(GBK)或3(UTF-8)位
	 * 
	 * @param str 源字符串
	...
	public static String substring(Object str, int beginIndex, int endIndex) {
		...
		for (int i = 0, len = value.length(); i < len; i++) {
			...
			c = String.valueOf(value.charAt(i));
			l = length(c);
			prevIndex = index;
			index += l;
			if (prevIndex < beginIndex) continue;
	...
	/**
	 * 从beginIndex开始截取，注意此处字符串的长度为其字节长度，即一个中文字符占2(GBK)或3(UTF-8)位
	 * 
	 * @param str 源字符串
	 * @param beginIndex
	 * @see #length(Object)
	...
	/**
	 * 保留左边length位，注意此处字符串的长度为其字节长度，即一个中文字符占2(GBK)或3(UTF-8)位
	 * 
	 * @param str 源字符串
	 * @param length 截取的长度
	 * @see #substring(Object, int, int)
	...
	/**
	 * 保留右边length位，注意此处字符串的长度为其字节长度，即一个中文字符占2(GBK)或3(UTF-8)位
	 * 
	 * @param str 源字符串
	 * @param length 截取的长度
	 * @see #length(Object)
	...
	public static void main(String[] args) {
		String ha = StringHelper.copy("哈哈", 2000);
		ha = StringHelper.substringLeft(ha, 2000);
		System.out.println(ha);
		System.out.println(ha.length());
		System.out.println(length(ha));
		// for (int i = 0; i < 10; i++)
