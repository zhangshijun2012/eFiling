...
public class UserService extends ServiceSupport<User, UserDao> {
	...
	/** 查询人员在eFiling的权限部门SQL */
	public static final String QUERY_GRADE_DEPARTMENT_SQL;
	static {
		StringBuilder b = new StringBuilder(100);
		b.append("SELECT * FROM (");
		b.append("SELECT DISTINCT COMCODE FROM UTIUSERGRADE WHERE (GRADECODE IN (?) OR GRADECODE IN (");
		...
		b.append("		( SELECT TASKCODE FROM UTIMENU WHERE SYSTEMCODE = ? AND VALIDSTATUS = ? ) ");
		b.append(")) AND USERCODE = ? ORDER BY COMCODE ");
		b.append(") WHERE ROWNUM = 1");
		QUERY_GRADE_DEPARTMENT_SQL = b.toString();
	}
	...
	public UserSessionEntity getSessionEntity(String userNo) {
		...
		String deptId = null;
		try {
			deptId = (String) getDao().uniqueResultSQL(
					QUERY_GRADE_DEPARTMENT_SQL,
					new Object[] {
							SystemUtils.GRADE_CODE_VIEW_DISABLED_FILE, // 407
							SystemUtils.SYSTEM_CODE, SystemUtils.STATUS_VALID, SystemUtils.SYSTEM_CODE,
							SystemUtils.STATUS_VALID, user.getId() });
		} catch (Exception e) {
			// TODO
		}
		if (deptId == null) { // 没有权限,可能是外部系统链入的地址,则使用user的归属部门
			deptId = user.getDepartment().getId();
			// return null; // 没有权限
		}

		UserSessionEntity userSupport = changeCurrentDepartment(user, deptId);
		return userSupport;
	}
	...
	public UserSessionEntity changeCurrentDepartment(User user, String departmentId) {
		...
		for (UserGrade userGrade : user.getUserGrades()) {
			...
			if (!SystemUtils.GRADE_CODE_VIEW_DISABLED_FILE.equalsIgnoreCase(grade.getId())
					&& null == dao.uniqueResultSQL(GRADE_EXISTS_SQL, parameters)) continue; // 此岗位没有电子档案的权限
