var Dialog = Class.create({
	...
	paint: function(options) {
		...
		// 判断footer是否被隐藏
		// alert(height +"," + this.footer.offsetTop + "," + this.footer.offsetHeight)
		...
		if (overflowY > 0 && overflowY < 22 && autoHeight && this.dialog.offsetHeight < maxHeight) {
			...
			height = this.dialog.offsetHeight;
			//alert('Y' + height + "," + overflowY + "," + maxHeight);
			...
			if (newHeight > height) {
				...
				this.body.style.height = (this.dialog.offsetHeight - this.header.offsetHeight - this.footer.offsetHeight) + 'px';
			}
		} else if (autoHeight) {	// 自动高度时缩小高度
			height = this.context.offsetHeight + 26;
			if (height < this.body.offsetHeight) {
				var newHeight = height + this.header.offsetHeight + this.footer.offsetHeight;
