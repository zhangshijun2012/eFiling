...
var Paging = {
	...
	selector: function(selector, cell, value, service, values) {
		...
		if (Object.isFunction(selector)) {
			v = selector.apply(service, [cell, value, values]);
			if (v != this.$skip) return v;
			selector = true;
		}
		if (selector === true) {
			if (!value && !values) cell.innerHTML = '<input type="checkbox" onclick="Main.service[\'checkAll\'](this.checked)" id="checkAllSelector" />';
	...
	list: function(data, service) {
		...
		data.list.each(function(values, index) {
			...
			if (selector) {
				var cell = row.insertCell();
				var selectorResult = me.selector(selector, cell, id, service, values);
				if (selectorResult !== false) service.selectorCount = service.selectorCount + 1;
			}
